...
TextCondReturn = namedtuple("TextCondReturn", ["embed", "mask"])
...
def classifier_free_guidance(
    ...
    auto_handle_text_condition = (
        texts_key_name not in fn_params and text_embeds_key_name not in fn_params
    )
    ...
    def inner(
        ...
        *args,
        ...
        cfg_routed_kwargs: Dict[
            str, Tuple[Any, Any]
        ] = dict(),  # to pass in separate arguments to forward and nulled forward calls (for handling caching when using CFG on transformer decoding)
        **kwargs,
    ):
        @wraps(fn)
        def fn_maybe_with_text(self, *args, **kwargs):
            if auto_handle_text_condition:
                ...
                assert not exists(cond_drop_prob) or 0.0 <= cond_drop_prob <= 1.0
                ...
                if exists(texts) ^ exists(text_embeds):
                    ...
                    assert exists(text_conditioner) and is_bearable(
                    ...
                    text_condition_input = (
                        dict(texts=texts)
                        if exists(texts)
                        else dict(text_embeds=text_embeds)
                    )

                    cond_fns, raw_text_cond = text_conditioner(
                        **text_condition_input, cond_drop_prob=cond_drop_prob
                    )

                elif isinstance(text_conditioner, NullConditioner):
                    assert (
                        cond_drop_prob == 0.0
                    ), "null conditioner has nothing to dropout"
        ...
        if self.training:
            assert (
                cond_scale == 1
            ), "you cannot do condition scaling when in training mode"

            return fn_maybe_with_text(self, *args, **kwargs)

        assert (
            cond_scale >= 1
        ), "invalid conditioning scale, must be greater or equal to 1"

        kwargs_without_cond_dropout = {**kwargs, cond_drop_prob_keyname: 0.0}
        kwargs_with_cond_dropout = {**kwargs, cond_drop_prob_keyname: 1.0}
        ...
        outputs = fn_maybe_with_text(
            self, *args, **fn_kwargs, **kwargs_without_cond_dropout
        )
        ...
        null_outputs = fn_maybe_with_text(
            self, *args, **null_fn_kwargs, **kwargs_with_cond_dropout
        )
        ...
        if rescale_phi <= 0:
            ...
            logit_output = rescaled_logits * rescale_phi + scaled_logits * (
                1.0 - rescale_phi
            )
...
def classifier_free_guidance_class_decorator(
    ...
    def max_cond_text_len(self):
        total_cond_text_len = sum(
            [
                text_model.max_text_len
                for text_model in self.text_conditioner.text_models
            ]
        )
...
class CrossAttention(nn.Module):
    def __init__(self, dim, hidden_dim, heads=8, dim_head=64, flash=False):
        super().__init__()
        self.attn = Attention(
            dim=hidden_dim,
            dim_context=dim,
    ...
    def forward(self, condition, hiddens, mask=None):
        return self.attn(hiddens, condition, mask=mask) + hiddens


# film text conditioning

CONDITION_CONFIG = dict(t5=T5Adapter, clip=OpenClipAdapter, bge=BGEAdapter)
...
class TextConditioner(Conditioner):
    def __init__(
        ...
        *,
        ...
        self.hiddens_channel_first = cast_tuple(
            hiddens_channel_first, self.num_condition_fns
        )  # whether hiddens to be conditioned is channel first or last
        ...
        self.text_embed_stem_mlp = nn.Sequential(
            nn.Linear(total_latent_dim, mlp_stem_output_dim), nn.SiLU()
        )
        ...
        self.register_buffer("_device_param", torch.tensor(0.0), persistent=False)
    ...
    def forward(
    ...
    ) -> Tuple[Tuple[Callable, ...], TextCondReturn]:
        ...
        if self.training:
            ...
            assert exists(
                cond_drop_prob
            ), "when not training, cond_drop_prob must be explicitly set"
        ...
        if cond_drop_prob > 0.0:
            prob_keep_mask = prob_mask_like(
                (batch, 1), 1.0 - cond_drop_prob, device=self.device
            )
            null_text_embeds = rearrange(self.null_text_embed, "d -> 1 d")

            text_embeds = torch.where(prob_keep_mask, text_embeds, null_text_embeds)
        ...
        for cond, cond_hiddens_channel_first, cond_repeat_batch in zip(
            self.conditioners, self.hiddens_channel_first, repeat_batch
        ):
            cond_text_embeds = repeat(
                text_embeds, "b ... -> (b r) ...", r=cond_repeat_batch
            )
            cond_fn = partial(cond, cond_text_embeds)

            wrapper_fn = (
                rearrange_channel_first
                if cond_hiddens_channel_first
                else rearrange_channel_last
            )
...
class AttentionTextConditioner(Conditioner):
    def __init__(
        ...
        *,
        ...
        dim_latent = default(
            dim_latent, max([model.dim_latent for model in text_models])
        )
        ...
        for hidden_dim in hidden_dims:
            self.conditioners.append(
                CrossAttention(dim_latent, hidden_dim, flash=flash)
            )
    ...
    def embed_texts(self, texts: List[str]):
        ...
        for text_model, to_latent in zip(self.text_models, self.to_latent_dims):
            ...
            text_embed = text_embed.masked_fill(~mask[..., None], 0.0)
    ...
    def forward(
        ...
        for cond, cond_hiddens_channel_first, cond_repeat_batch in zip(
            ...
            cond_mask = (
                repeat(mask, "b ... -> (b r) ...", r=cond_repeat_batch)
                if exists(mask)
                else None
            )
